...
### 编辑IR代码
虚拟机实例界面的左侧是IR代码编辑器，它使用的是与VSCode同款的[Monaco Editor](https://github.com/microsoft/monaco-editor)，支持IR代码的语法高亮、自动补全、搜索（`Ctrl+F`）、替换（`Ctrl+H`）、切换注释（`Ctrl+/`）与可视化错误提示等多种功能。您可以在其中编辑IR代码，并且方便地发现和修正IR代码的静态检查错误。
