...
运行示例IR程序中的`Benchmark`，**本虚拟机执行步数为`3000004`，耗时约为`92`毫秒，合计约`3260.9`万指令/秒。**  
作为对比，使用irsim运行同一程序（硬件和OS环境相同，Python 3.8.19），执行步数`3000006`，耗时约`18.5`秒，合计仅约`16.2`万指令/秒。**本虚拟机性能高于irsim约`200`倍。**
