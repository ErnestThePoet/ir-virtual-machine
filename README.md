...
注：示例IR程序及其cmm源代码位于仓库的`public\examples`目录下
...
## 虚拟机性能
作者的电脑是64位Windows 10操作系统，CPU是i5-11260H，浏览器是Edge 112.使用示例IR程序中的`Benchmark`进行测试时，本虚拟机执行步数为`990004`，耗时约为`1.05`秒，合计约`94.3`万指令/秒；irsim执行步数`990006`，耗时约`6.2`秒，合计约`16.0`万指令/秒。
